...
public class GeradorAlteracaoFormCalango extends CalangoFactory {

    public static void build(EAProjectConfig projeto) throws IOException {

	for (Entity entidade : projeto.getEntidades()) {
	    String nomePagina = entidade.getNomeEntidade().toLowerCase() + "_update.xhtml";
	    ...
	    srtPage.append("<ui:define name=\"caminho\">" + entidade.getNomeEntidade() + " - Altera��o</ui:define> \n\t\t");
	...
	// String caminhoPaginas = projeto.getDestinoView() +
	// projeto.getNomeSistema().toLowerCase() + "/"+
	// entidade.getNomeEntidade().toLowerCase();
	...
	srtPage.append("<ui:define name=\"header\">Altera��o de " + entidade.getNomeEntidade() + "</ui:define> \n\t");
	...
	srtPage.append("<s:decorate id=\"botoes_box\" template=\"/layout/util/box_button.xhtml\"> \n\t\t\t\t\t");
