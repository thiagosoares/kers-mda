...
public class GeradorBuscasCalango extends CalangoFactory {
	...
	for (Entity entidade : projeto.getEntidades()) {
	    ...
	    srtPage.append("<ui:define name=\"titulo\">" + entidade.getNomeEntidade() + "</ui:define> \n\t");
	    srtPage.append("<ui:define name=\"caminho\">" + entidade.getNomeEntidade() + " - Cadastro</ui:define> \n\t");
	    srtPage.append("<ui:define name=\"pesquisa\"> \n\t\t");
	    srtPage.append("<h:form id=\"" + entidade.getNomeEntidade().toLowerCase() + "_form_pesq\"> \n\t\t\t");
	    ...
	    srtPage.append("<ui:include src=\"include/" + entidade.getNomeEntidade().toLowerCase() + "_list.xhtml\" /> \n\t\t\t");
	    srtPage.append("<ui:include src=\"include/" + entidade.getNomeEntidade().toLowerCase() + "_delete.xhtml\" /> \n\t\t");
	    srtPage.append("</h:form> \n\t\t");
	    srtPage.append("<ui:include src=\"include/" + entidade.getNomeEntidade().toLowerCase() + "_view.xhtml\" /> \n\t");
	    srtPage.append("</ui:define>\n");
	    srtPage.append("</ui:composition> \n");
	...
	String nomePagina = entidade.getNomeEntidade().toLowerCase() + "_search.xhtml";
	...
	srtPage
		.append("<ui:composition" +
	          ...
	          " \n\t xmlns:rich=\"http://richfaces.org/rich\" \n\t" +
	          " \n\t template=\"/layout/util/panel_2.xhtml\"> \n\t");
	srtPage.append("\n\t");
	...
	srtPage.append("<ui:define name=\"body_pesquisa\"> \n\t\t");
	...
	srtPage.append("\n \t");
	srtPage.append("<ui:define name=\"botoes\"> \n\t\t");
	...
	srtPage.append("<s:link action=\"#{" + entidade.getNomeVarAction() + ".formConversation}\" view=\"/"
		+ projeto.getNomeSistema().toLowerCase() + "/" + entidade.getNomeEntidade().toLowerCase() + "/"
		+ entidade.getNomeEntidade().toLowerCase() + "_form.xhtml\" title=\"Novo " + entidade.getNomeEntidade() + "\"> \n \t\t\t");
	...
	String nomePagina = entidade.getNomeEntidade().toLowerCase() + "_list.xhtml";
	...
	srtPage.append("Nenhum " + entidade.getNomeEntidade() + " foi encontrado. \n\t\t");
	...
	srtPage.append("<a4j:commandLink id=\"showItem\" action=\"#{" + entidade.getNomeVarAction()
			+ ".selecionar}\" reRender=\"consulta\" onclick=\"javascript:Richfaces.showModalPanel('mp',{width:420, top:200});\" title=\"Visualizar "
			+ entidade.getNomeEntidade() + "\"> \n\t\t\t\t\t");
	...
	srtPage.append("<h:outputText value=\"\"/> \n\t\t\t\t");
	srtPage.append("</f:facet> \n\t\t\t\t");
	srtPage.append("<s:link action=\"#{" + entidade.getNomeVarAction() + ".selecionar}\"  view=\"/" + projeto.getNomeSistema().toLowerCase()
		+ "/" + entidade.getNomeEntidade().toLowerCase() + "/" + entidade.getNomeEntidade().toLowerCase()
		+ "_alteracao.xhtml\" title=\"Alterar " + entidade.getNomeEntidade() + "\"> \n\t\t\t\t\t");
	...
	srtPage.append("<a4j:commandLink id=\"deleteItem\" reRender=\"exclusao\" action=\"#{" + entidade.getNomeVarAction()
		+ ".selecionar}\" onclick=\"javascript:Richfaces.showModalPanel('mpexcluir',{width:420, top:200});\"  title=\"Excluir "
		+ entidade.getNomeEntidade() + "\"> \n\t\t\t\t\t");
	...
	srtPage.append(" \n\t\t");
	srtPage.append("<br/> \n\t\t");
	srtPage.append("<rich:datascroller id=\"paginador\" for=\"" + entidade.getNomeVarEntidade().toLowerCase() + "Table\" maxPages=\"5\" rendered=\"#{" + entidade.getNomeVarDto()
		+ "Lista.rowCount > 10}\" /> \n\t\t");
	srtPage.append(" \n\t");
	...
	String nomePagina = entidade.getNomeEntidade().toLowerCase() + "_view.xhtml";
	...
	srtPage.append("template=\"/layout/util/modal_consulta.xhtml\"> \n\t");
	srtPage.append("\n\t");

	srtPage.append("<ui:define name=\"header\">" + entidade.getNomeEntidade() + "</ui:define> \n\t");
	...
	String nomePagina = entidade.getNomeEntidade().toLowerCase() + "_delete.xhtml";
	...
	srtPage.append("<ui:define name=\"body\"> \n\t\t\t");
	srtPage.append("<rich:modalPanel id=\"mpexcluir\" height=\"140\" width=\"470\" zindex=\"2000\"> \n\t\t");

	srtPage.append("<f:facet name=\"header\"> \n\t\t\t");
	srtPage.append("<h:outputText value=\"Excluir Tipo de Origem de Bens \"/> \n\t\t");
	srtPage.append("</f:facet>\n\t\t");

	srtPage.append("<f:facet name=\"controls\">\n \t\t\t");
	srtPage
		.append("<h:graphicImage value=\"../../images/ico_fecharjanela.gif\" style=\"cursor:pointer\" onclick=\"Richfaces.hideModalPanel('mp')\" /> \n \t\t");
	srtPage.append("</f:facet> \n \t\t");
	...
	if (idField != null) {
	    ...
	    srtPage.append("<ui:define name=\"label\"><h:outputText value=\"#{" + entidade.getNomeVarDto() + "." + idField
		    + "}\" />:</ui:define> \n\t\t\t");
	    srtPage.append("</s:decorate>\n\t\t\t");
	} else {
	    srtPage.append("<s:decorate template=\"../../../layout/util/display_view.xhtml\"> \n\t\t\t\t");
	    srtPage.append("<ui:define name=\"label\">IDENTIFICADOR:</ui:define> \n\t\t\t\t");
	    srtPage.append("<ui:define name=\"label\"><h:outputText value=\"#{" + entidade.getNomeVarDto()
		    + ".IDENTIFICADOR}\" />:</ui:define> \n\t\t\t");
	    srtPage.append("</s:decorate>\n\t\t\t");
	}
	...
	srtPage.append("<a4j:commandButton id=\"confirmaExclusao\" value=\"Confirmar Exclus�o\" styleClass=\"botao\" action=\"#{"+ entidade.getNomeVarAction() +".excluir}\" oncomplete=\"Richfaces.hideModalPanel('mpexcluir')\" reRender=\"" + entidade.getNomeEntidade().toLowerCase() + "_form_list\"/> \n \t\t");
	srtPage.append("<a4j:commandButton id=\"cancelaExclusao\" value=\"Cancelar Exclus�o\" styleClass=\"botao\" onclick=\"Richfaces.hideModalPanel('mpexcluir')\" /> \n \t");
	srtPage.append("</h:panelGrid> \n \t");
	srtPage.append(" \n \t");
	srtPage.append(" \n \t");
	...
	for (Attribute atributo : entidade.getAtributos()) {
		...
		srtPage.append("<s:decorate id=\"" + nome + "Field\" template=\"/layout/util/display_view.xhtml\"> \n\t\t\t");
		srtPage.append("<ui:define name=\"label\">" + nome + ":</ui:define> \n\t\t\t");
		srtPage.append("<ui:define name=\"label\"><h:outputText value=\"#{" + entidade.getNomeVarDto() + "." + nome
			+ "}\" /></ui:define> \n\t\t");
		srtPage.append("</s:decorate>\n\t\t");
		break;
	...
	for (Attribute atributo : entidade.getAtributos()) {
	    ...
	    case 1: // Strings
		srtPage.append("<h:outputText value=\" " + nome + ":\" style=\"componente_label_view\" /> \n\t\t");
	    ...
	    case 6: // TipoBoleano da prodepa
		srtPage.append("<h:outputText value=\" " + nome + ":\" style=\"componente_label_view\" /> \n\t\t");
		srtPage.append("<h:outputText value=\"#{" + entidade.getNomeVarDto() + "." + nome
			+ ".descricao}\" style=\"componente_label_view\" /> \n\t\t");
		break;
